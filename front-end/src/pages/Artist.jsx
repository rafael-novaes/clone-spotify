...
const Artist = () => {
  const {id} = useParams()
  const {name, banner} = artistArray.filter((currArtistObj) => currArtistObj._id === id)[0]

  const songsArrayFromArtist = songsArray.filter((currSongsObj) => currSongsObj.artist === name)
